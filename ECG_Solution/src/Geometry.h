...
public:
	 ...
	 * Uses the animation shader, sets the uniform and issues a draw call
	 */
	...
	* @param dimensions: for example 10 creates a 10x10 plane with 100vertices
	* @return all plane data
	*/
	static GeometryData createPlaneGeometry(float dimensions);
	...
	/*!
	 * Creates a Trap the player can fall into
	 * @param width: width of the hole
	 * @param height: height of the hole
	 * @param depth: depth of the hole
	 * @return all trap data
	 */
	static GeometryData createHoleTrapGeometry(float width, float height, float depth);
};