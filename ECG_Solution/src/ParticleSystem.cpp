...
ParticleSystem::ParticleSystem(Shader& shader, Texture& texture,Camera& cam, float offsetFactor,float size, unsigned int amount,glm::vec3 position)
    : _shader(&shader), _texture(&texture), _camera(&cam) ,_amount(amount), _offsetFactor(offsetFactor),_size(size),_position(position){
...
void ParticleSystem::Draw()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, _particles_position_buffer);
    glBufferData(GL_ARRAY_BUFFER, _amount * 4 * sizeof(GLfloat), NULL, GL_STREAM_DRAW); // Buffer orphaning, a common way to improve streaming perf. See above link for details.
    ...
    _texture->bind(0);
    glUniform1i(_shader->getUni("sprite"), 0);  
    _shader->setUniform("viewProjection", _camera->getProjectionMatrix()*_camera->GetViewMatrix());
    _shader->setUniform("camRight", _camera->getRight());
    _shader->setUniform("camUp", _camera->getUp());
