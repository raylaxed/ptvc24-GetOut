...
void Geometry::draw(Shader* shader)
{
	...
	shader->use();
	shader->setUniform("lightColor", glm::vec3(0.902f, 0.376f, 0.118f));
	shader->setUniform("modelMatrix", _modelMatrix);
	shader->setUniform("normalMatrix", glm::mat3(glm::transpose(glm::inverse(_modelMatrix))));
	//_material->setUniforms();
...
GeometryData Geometry::createPlaneGeometry(float dimensions)
{
	GeometryData data;
	...
	*/
	float half = dimensions / 2;

	//VERT
	for (int w = 0; w < dimensions; w++) {
		for (int l = 0; l < dimensions; l++) {
			data.positions.push_back(glm::vec3(l - half, 0.f, w - half));
	...
	for (int row = 0; row < dimensions - 1; row++) {
		for (int col = 0; col < dimensions - 1; col++) {
			data.indices.push_back(dimensions * row + col);
			data.indices.push_back(dimensions * row + col + dimensions);
			data.indices.push_back(dimensions * row + col + dimensions + 1);

			data.indices.push_back(dimensions * row + col + dimensions + 1);
			data.indices.push_back(dimensions * row + col + 1);
			data.indices.push_back(dimensions * row + col);

			data.uvs.push_back(glm::vec2(0, 1));
			data.uvs.push_back(glm::vec2(0, 0));
			data.uvs.push_back(glm::vec2(1, 0));
			data.uvs.push_back(glm::vec2(1, 1));
		}
	}
...
GeometryData Geometry::createHoleTrapGeometry(float width, float height, float depth)
{
	GeometryData data;
