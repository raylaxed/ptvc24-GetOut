...
void main()
{
	...
	vec3 albedo = vec3(pow(texture(baseColorTex, vert.uv).r, 2.2),
					   pow(texture(baseColorTex, vert.uv).g, 2.2),
					   pow(texture(baseColorTex, vert.uv).b, 2.2));
	float metallic = texture(metallicTex,vert.uv).r;
	float roughness = texture(roughnessTex,vert.uv).r;
	float ao = texture(aoTex,vert.uv).r;
	
	
	vec3 N = normalize(vert.normal_world);
	vec3 viewDir = normalize(camera_world-vert.position_world);

	vec3 F0New = vec3(0.004f);
	...
	for(int i = 0; i < NR_POINT_LIGHTS; i++)
	{
		...
		float NDF = GGXdistribution(roughness,NdotH);
		float G = ggx1*ggx2;
